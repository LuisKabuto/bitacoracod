...
(function(B){
  let seq=3;
  const incidents=[
    {id:'i1',title:'Intermitencia de conexión',category:'Conectividad',priority:'high',status:'open',assignedTo:'u_osle',assignedName:'Osledy Avila',description:'Revisar estabilidad de la conexión durante la jornada.',createdBy:'u_admin',createdAt:new Date()},
  ...
  B.services.incidents={
    ...
    create(data){incidents.unshift({...data,id:'i'+seq++,createdAt:new Date()});emit();return Promise.resolve()},
    update(id,status){const item=incidents.find(x=>x.id===id);if(!item)return Promise.resolve();item.status=status;emit();return Promise.resolve()}
  };
})(window.Bitacora);
